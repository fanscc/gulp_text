...
gulp.task('imgsprit', function () {
    return gulp.src('login/static/img/*.{png,gif}')//需要合并的图片地址
        .pipe(spritesmith({
...
gulp.task('browser-sync', function () {
    gulp.watch(['login/static/css/*.css']).on('change',function(files){
        // 1. 找到文件
        gulp.src(files.path)
        // 2. 压缩文件
    //    .pipe(minifyCSS())
    ...
    gulp.watch(['login/static/js/*.js']).on('change',function(files){
...
gulp.task('revHtml', function () {
  return gulp.src(['login/rev/**/*.json', 'login/static/*.html'])
    .pipe(revCollector())
